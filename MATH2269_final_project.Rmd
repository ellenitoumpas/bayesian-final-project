...
# Set temporary values for subsampling trials 
training_data <- data_cleaned %>% 
  ...
  select('rf_cum_3_day','temperature', 'ws', 'deg_from_north', 'dow', 'working_days', 'hour', 'pre_peak_hour', 'pm10') %>% 
...
prediction_data <- data_cleaned %>% 
  mutate(pm10 = if_else(pm10 < 0, 0, pm10)) %>% 
  filter(date_day %in% c(ymd('2018-12-28'), ymd('2018-12-29'), ymd('2018-12-30'), ymd('2018-12-31'))) %>% 
  select('rf_cum_3_day','temperature', 'ws', 'deg_from_north', 'dow', 'working_days', 'hour', 'pre_peak_hour')   %>% 
  mutate(dow = as.numeric(dow), 
         working_days = as.numeric(working_days), 
         pre_peak_hour = as.numeric(pre_peak_hour))

# Select variables
# training_independant_data <- training[,c('rf_cum_3_day','temperature', 'ws', 'deg_from_north', 'dow', 'working_days', 'hour', 'pre_peak_hour', 'pm10')]
...
xPred = as.matrix(array(NA, dim = c(nrow(prediction_data), length(colnames(prediction_data)))))
