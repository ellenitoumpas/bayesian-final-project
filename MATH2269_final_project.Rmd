...
To begin the analysis we first import the Brooklyn air data. The column names are cleaned and the ordinal variables are converted to factors with their levels and orders set.

### A descriptive look

Before commencing with the analysis a descriptive look at the data is performed. This enables us to capture an understanding of the data and address any data quality issues. 

#### Dimensions

In Table **xx** below we can see the original data contains `r dim(data_cleaned)[2]` columns and `r dim(data_cleaned)[1]` records.
...
In Table **xx** below we can see the first few records from the imported dataset.
...
TODO: Talk about what target variables we had the choice of and why we chose these. 
...
data_cleaned <- data_cleaned %>% mutate(pm10 = if_else(pm10 < 0 , 0, pm10))
...
##### Any categorical descriptive feature encoded/ Feature engineering
...
```{r Split features into training and test, echo=FALSE, message=FALSE, warning=FALSE}

# Moved to constants.R?
...
```{r Set values for all trials, echo=FALSE, message=FALSE, warning=FALSE}
...
# xPred <- set_prediction_matrix(prediction_data, pred_values = c(1, 2, 10, 20, 30, 40, 50, 60))
# 
# # Parameters to use
...
#   number_adaptation_steps = number_adaptation_steps,
...
```{r Subsample size trials, echo=FALSE, message=FALSE, warning=FALSE}
...
TODO: After talking about the model and reconfirming the likelihood type we need to talk about how the pm10 levels can not be 0 (as gamma APPROACHES 0.) When running the JAGS model WITH 0s it caused issues, therefore a decision was made to add 0.01 to all pm10 readings.
...
# Parameters to use
use_parameters <- list(number_chains = 3, 
